...
`OptionsQuotesResponse` wrapping `IReadOnlyList<OptionQuote>` — a single row for a live quote, or one row per trading day for a historical window. See [chain](./chain.md#returns) for the full `OptionQuote` record.
...
For CSV output, call `client.Options.GetQuoteCsvAsync(...)` and read `.Csv`. See [Settings](../settings.md#csv-output).
