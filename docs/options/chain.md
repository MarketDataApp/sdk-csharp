...
`OptionsChainResponse` wrapping `IReadOnlyList<OptionQuote>` — the same record the [quotes](./quotes.md) endpoint returns, one per contract:
...
For CSV output, call `client.Options.GetChainCsvAsync(...)` and read `.Csv`. See [Settings](../settings.md#csv-output).
