...
Both accept an optional `MarketDataClientOptions`. When omitted, options are loaded from the [configuration cascade](./settings.md#configuration-cascade) (user secrets, `.env`, environment variables).
...
Both overloads also accept an optional [`MarketDataRequestOptions`](./settings.md#universal-parameters) for the universal parameters (date format, mode, columns, limit, offset).
